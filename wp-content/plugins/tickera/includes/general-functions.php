...
function tc_get_tickets_user_purchased_count( $user_id, $ticket_type_id = null) {
    global $wpdb;

    if ( !$user_id )
        return false;

    $query = $wpdb->prepare("SELECT `post_content` FROM {$wpdb->posts} WHERE `post_type` = 'tc_orders' AND `post_author` = %s AND `post_status` IN ('order_paid', 'order_received', 'order_fraud') AND `post_content` <> ''", $user_id);
...
function tc_get_tickets_count_sold($ticket_id) {
    ...
    $tc_general_settings = get_option( 'tc_general_setting', false );
    $removed_cancelled_orders_from_stock = isset( $tc_general_settings['removed_cancelled_orders_from_stock'] ) ? ( ( 'yes' == $tc_general_settings['removed_cancelled_orders_from_stock'] ) ? true : false ) : true;

    $skip_statuses = ( $removed_cancelled_orders_from_stock ) ? array( 'trash', 'order_cancelled', 'order_refunded' ) : array( 'trash' );

    $query = $wpdb->prepare( "SELECT COUNT(*) as cnt, p.post_parent FROM {$wpdb->posts} p, {$wpdb->postmeta} pm WHERE p.ID = pm.post_id AND p.post_status = 'publish' AND pm.meta_key = 'ticket_type_id' AND pm.meta_value = %s GROUP BY p.post_parent", (int) $ticket_id );
...
function tc_order_paid_attendee_email($order_id) {
    ...
    if (!isset($tc_email_settings['attendee_send_message']) || (isset($tc_email_settings['attendee_send_message']) && $tc_email_settings['attendee_send_message'] == 'yes')) {
        ...
        foreach ($order_attendees as $order_attendee_id) {
            ...
            $ticket_code = strtoupper( get_post_meta( $order_attendee_id, 'ticket_code', true) );
            ...
            $placeholders = array('EVENT_NAME', 'DOWNLOAD_URL', 'TICKET_TYPE','FIRST_NAME','LAST_NAME', 'EVENT_LOCATION');

            $placeholder_values = array($event->details->post_title, tc_get_ticket_download_link('', '', $order_attendee_id, true), $ticket_type_name,$first_name,$last_name,$event_location);
            ...
            if (!empty($to)) {
                ...
                if ($email_send_type == 'wp_mail') {
                    ...
                    wp_mail( $to, $subject, html_entity_decode( stripcslashes( apply_filters('tc_order_completed_attendee_email_message', wpautop( $message ) ) ) ), apply_filters( 'tc_order_completed_attendee_email_headers', $attendee_headers ), $attachment );
    ...
    if ( '' !== $phpmailer->ErrorInfo && isset( $_POST['ticket_code'] ) ) {
        ...
        if ( false !== stripos( $error_info, $translations['file_access'] ) ) {
            ...
            if ( isset( $attachment ) && $attachment ) {
                ...
                // Retrieve file name via post
                $file_name = ( isset( $_POST['ticket_code'] ) ) ? sanitize_title( $_POST['ticket_code'] ) . '.pdf' : 'ticket.pdf';
                unset( $_POST['ticket_code'] );
...
function tc_order_created_email($order_id, $status, $cart_contents = false, $cart_info = false, $payment_info = false, $send_email_to_admin = true) {
    ...
    if ( 'order_paid' == $status ) {
        ...
        if ( !isset( $tc_email_settings['client_send_message'] ) || ( isset( $tc_email_settings['client_send_message'] ) && 'yes' == $tc_email_settings['client_send_message'] ) ) {
            ...
            $order_clients = TC_Orders::get_tickets_ids( $order->details->ID );
            ...
            $placeholder_values = array( $order_id, apply_filters('tc_cart_currency_and_format', $payment_info['total'] ), $order_status_url, $buyer_name, tc_get_order_details_email( $order->details->ID, $order->details->tc_order_date, true, $status ), implode( ' | ', $event_titles ), implode( ' | ', $event_locations ) );
         ...
         */
        tc_order_paid_attendee_email( isset( $order->ID ) ? $order->ID : $order->details->ID );
...
function extended_radio_button( $field_name, $values, $value = '' ) {
    ...
    foreach( $values as $key => $val ) {
        ...
        $html .= '<label>';
        $html .= '<input type="radio" class="' . esc_attr( $field_name ) . '" name="' . esc_attr( $field_name ) . '" value = "' . $val . '" ' . $checked . '/>' . __( ucfirst( $val ), 'tc' );
        $html .= '</label>';
    }
    echo $html;
}
...
function tc_get_order_status_select( $field_name = '', $post_id = '' ) {
    ...
    <select class="order_status_change" name="order_status_change">
        ...
        <option value='order_refunded' <?php selected($value, 'order_refunded', true); ?>><?php _e('Order Refunded', 'tc'); ?></option>
        <?php if ($value == 'trash') { ?>
...
function tc_get_user_roles( $field_name = '', $post_id = '' ) {
    ...
    <select name="<?php echo esc_attr($field_name); ?>_post_meta[]" multiple="true" id="tc_ticket_types">
        ...
        <?php foreach ($user_roles_object->roles as $key => $value) : ?>
            <?php
            $role_id = esc_attr( $key );
            $role_name = esc_attr( $value['name'] );
            ?>
            <option value="<?php echo $role_id ?>" <?php echo is_array($currently_selected) && in_array($role_id, $currently_selected) ? 'selected' : ''; ?>><?php echo $role_name; ?></option>
        <?php endforeach; ?>
    </select>
