...
* @fs_ignore /css/, /js/, /languages/, /fonts/, /images/, /includes/addons/, /includes/admin-pages/, /includes/classes/, /includes/network-admin-pages/, /includes/tcpdf/, /includes/templates/, /includes/templates/, /includes/ticket-elements/, /includes/widgets/
*/
if ( !defined( 'ABSPATH' ) ) {
    exit;
}
...
if ( !class_exists( 'TC' ) ) {
    class TC
    {
        ...
        function admin_dashboard_notification( $message = null, $notice_type = 'notice-info' )
        {
            ...
            $html = '';
            $html .= '<div class="notice ' . $notice_type . ' is-dismissible tc-admin-notice" style="display:none;">';
            $html .= '<p>' . $message . '</p>';
            $html .= '</div>';
            echo  $html ;
        }
        ...
        function tc_payment_gateway_alternative( $gateway_name, $notice_type = 'notice-info' )
        {
            ...
            if ( $alternative_gateway && in_array( $gateway_name, $active_gateways ) ) {
                foreach ( $alternative_gateway as $val ) {
                    $message = __( 'Tickera ', 'tc' ) . ucfirst( str_replace( '_', ' ', '<b>' . $val . '</b>' ) ) . __( ' is now available. Please activate it via', 'tc' ) . ' <a href="edit.php?post_type=tc_events&page=tc_settings&tab=gateways">' . __( 'Tickera > Settings > Payment Gateways Tab', 'tc' ) . '</a>';
    ...
    <?php 
        ...
        function tc_recalculate_order_totals( $order_id )
        {
            ...
            update_post_meta( $order_id, 'tc_cart_contents', $cart_contents );
            ...
            update_post_meta( $order_id, 'tc_cart_info', $order_cart_info );
            update_post_meta( $order_id, 'tc_payment_info', $order_payment_info );
            return $cart_totals;
        }
        ...
        function save_attendee_info()
        {
            ...
            if ( isset( $_POST['post_id'] ) && isset( $_POST['meta_name'] ) && isset( $_POST['meta_value'] ) ) {
                ...
                $order_id = get_post( $post_id )->post_parent;
                ...
                $ticket_quantity = ( 'tc_orders' == $post_type ? get_post_meta( $meta_value, 'quantity_available', true ) : get_post_meta( $meta_value, '_stock', true ) );
                // Woocommerce Orders
      ...
      <?php 
        ...
        function save_cart_post_data()
        {
            ...
            if ( isset( $_POST ) ) {
                ...
                $_SESSION['cart_info']['coupon_code'] = ( isset( $_POST['coupon_code'] ) ? sanitize_text_field( $_POST['coupon_code'] ) : '' );
      ...
      <?php 
        ...
        {
            global  $tc_cart_errors, $cart_error_number, $tc_cart_error_numbers ;
            $cart_error_number = 0;
            ...
            if ( isset( $_POST['cart_action'] ) && in_array( $_POST['cart_action'], $valid_cart_actions ) ) {
                $discount = new TC_Discounts();
                ...
                if ( in_array( $_POST['cart_action'], array( 'proceed_to_checkout', 'update_cart' ) ) ) {
                    ...
                    foreach ( $updated_cart_contents as $ticket_type_id => $qty_count ) {
                        ...
                        if ( $qty_count <= 0 ) {
                            ...
                            // Remove from cart
                            $tc_cart_error_numbers[$ticket_type_id]['errors'][] = 103;
                        } else {
                            ...
                            if ( $ticket->details->min_tickets_per_order && $qty_count < $ticket->details->min_tickets_per_order ) {
                                ...
                                $cart_error_number++;
                                $tc_cart_error_numbers[$ticket_type_id]['errors'][] = 100;
                            }
                            ...
                            if ( $ticket->details->max_tickets_per_order && $qty_count > $ticket->details->max_tickets_per_order ) {
                                ...
                                $cart_error_number++;
                                $tc_cart_error_numbers[$ticket_type_id]['errors'][] = 101;
                            }
                            ...
                            if ( $quantity_left >= $qty_count ) {
                                ...
                                $tc_cart_error_numbers[$ticket_type_id]['errors'][] = 102;
                            }
                            ...
                            if ( 'yes' == $force_login && isset( $ticket->details->max_tickets_per_user ) && $ticket->details->max_tickets_per_user && $user_purchased_count + $qty_count > $ticket->details->max_tickets_per_user ) {
                                ...
                                $cart_error_number++;
                                $tc_cart_error_numbers[$ticket_type_id]['errors'][] = 104;
                            }
                ...
                } elseif ( 'empty_cart' == $_POST['cart_action'] ) {
                    $this->update_cart_cookie( array() );
                } elseif ( 'apply_coupon' == $_POST['cart_action'] ) {
                    $discount = new TC_Discounts();
                    $discount->discounted_cart_total();
                    add_filter(
                ...
                if ( 'proceed_to_checkout' == $_POST['cart_action'] ) {
      ...
      <?php 
        ...
        function tc_order_confirmation_message_content( $content, $order )
        {
            ...
            if ( $order->details->post_status == 'order_refunded' ) {
                $content .= sprintf( __( 'You can check your order status %s.', 'tc' ), $order_status_url );
            }
        ...
        function load_cart_scripts()
        {
            ...
            if ( apply_filters( 'tc_use_cart_scripts', true ) == true ) {
                ...
                wp_localize_script( 'tc-cart', 'tc_ajax', array(
                    ...
                    'tc_field_error'             => __( 'This field required *', 'tc' ),
                ) );
            }
        ...
        {
            $id = ( is_int( $order_id ) ? $order_id : tc_get_order_id_by_name( $order_id ) );
            if ( empty($id) ) {
                return false;
            }
            $order = get_post( $id );
