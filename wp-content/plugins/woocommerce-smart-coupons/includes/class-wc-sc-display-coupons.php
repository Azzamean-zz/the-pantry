	...
	class WC_SC_Display_Coupons {
		...
		public function show_available_coupons( $available_coupons_heading = '', $page = 'checkout' ) {
			...
			<div id="coupons_list" style="display: none;">
				...
				?>
				<h3><?php echo __( stripslashes( $available_coupons_heading ), 'woocommerce-smart-coupons' ); // phpcs:ignore ?></h3>
					<div id="sc-cc">
		...
		public function attached_coupons_styles_and_scripts() {
			...
			<script type="text/javascript">
				jQuery(function(){
					let product_type = '<?php echo esc_js( $product_type ); ?>';
						...
						if ( product_id != '' && product_id != undefined ) {
							jQuery.ajax({
								...
								data: {
									...
									security: '<?php echo esc_js( wp_create_nonce( 'wc-sc-get-attached-coupons' ) ); ?>'
								},
					...
					setTimeout(function(){
						...
						if ( product_types.includes( product_type ) ) {
							...
							reload_attached_coupons( '<?php echo esc_js( $post->ID ); ?>' );
						}
					}, 10);
		...
		public function frontend_styles_and_scripts( $args = array() ) {
			...
			if ( $this->is_wc_gte_26() ) {
				$js .= "
						let sc_available_coupons_ajax = null;
