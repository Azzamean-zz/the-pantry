...
namespace FCFProVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use FCFProVendor\WPDesk\Helper\Debug\LibraryDebug;
use FCFProVendor\WPDesk\Helper\Integration\LicenseIntegration;
use FCFProVendor\WPDesk\Helper\Integration\LogsIntegration;
use FCFProVendor\WPDesk\Helper\Integration\SettingsIntegration;
use FCFProVendor\WPDesk\Helper\Integration\TrackerIntegration;
use FCFProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use FCFProVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \FCFProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \FCFProVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \FCFProVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \FCFProVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \FCFProVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \FCFProVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \FCFProVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        $library_debug_info = new \FCFProVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \FCFProVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \FCFProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
