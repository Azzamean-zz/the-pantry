...
use FCFProVendor\WPDesk\License\ServerAddressRepository;
use FCFProVendor\WPDesk_API_Manager_With_Update_Flag;
/**
 * Can activate plugin license.
 *
 * @package WPDesk\License\Page\License\Action
 */
class LicenseActivation implements \FCFProVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \FCFProVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        \add_settings_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-checkout-fields-pro'), 'error');
    }
    /**
    ...
    {
        \add_settings_error('activate_text', 'activate_msg', \__('Plugin activated. ', 'flexible-checkout-fields-pro') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \FCFProVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
