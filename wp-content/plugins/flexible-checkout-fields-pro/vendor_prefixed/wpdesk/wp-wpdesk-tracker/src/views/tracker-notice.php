...
if (empty($notice_content)) {
    ?>
            <?php 
    \printf(\__('Hey %s,', 'flexible-checkout-fields-pro'), $username);
    ?><br/>
            <?php 
    \_e('We need your help to improve <strong>WP Desk plugins</strong>, so they are more useful for you and the rest of <strong>30,000+ users</strong>. By collecting data on how you use our plugins, you will help us a lot. We will not collect any sensitive data, so you can feel safe.', 'flexible-checkout-fields-pro');
    ?>
            <a href="<?php 
    echo $terms_url;
    ?>" target="_blank"><?php 
    \_e('Find out more &raquo;', 'flexible-checkout-fields-pro');
    ?></a>
        <?php 
} else {
    ?>
...
\_e('Allow', 'flexible-checkout-fields-pro');
?></button>
    </p>
</div>
