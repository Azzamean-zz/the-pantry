(function($){var Timeselector=function(){this._$div=null;this._inst=null;this._showing=!1;this.uuid=new Date().getTime();this.initialized=!1};Timeselector.prototype={constructor:Timeselector,_generateHtml:function(){return'<div id="timeselector-div" class="timeselector">'+'<div class="timeselector-item timeselector-hour" data-type="hour">'+'<a class="timeselector-button timeselector-up">+</a>'+'<span class="timeselector-value"></span>'+'<a class="timeselector-button timeselector-down">-</a>'+'</div>'+'<div class="timeselector-separator">:</div>'+'<div class="timeselector-item timeselector-minute" data-type="minute">'+'<a class="timeselector-button timeselector-up">+</a>'+'<span class="timeselector-value"></span>'+'<a class="timeselector-button timeselector-down">-</a>'+'</div>'+'</div>'},_setTime:function(inst,step){var date=inst.date;if(step){date.setTime(+date+(1000*60*step))}
...
inst.lastValue=inst.input.val();var date=new Date(),matches,hour;matches=this._match(inst.lastValue);if(matches){hour=parseInt(matches[0],10);if(matches[2]){if(matches[2].toLowerCase()==='am'&&hour===12){hour=0}else if(matches[2].toLowerCase()==='pm'&&hour>0&&hour<12){hour=hour+12}}
date.setHours(hour);date.setMinutes(parseInt(matches[1],10))}
inst.date=date},_match:function(time){var rtime=/^((?:1[012]|0[1-9])|(?:[01][0-9]|2[0-3])):([0-5][0-9])(?:\s(am|pm))?$/i,matches=rtime.exec(time);if(matches){matches.splice(0,1)}
...
return this};$.fn.timeselector.defaults={hours12:!0,step:1};$.fn.timeselector.timer=new Timeselector()}(jQuery))