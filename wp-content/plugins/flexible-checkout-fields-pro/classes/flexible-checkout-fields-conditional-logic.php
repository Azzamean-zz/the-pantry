...
require_once 'Shipping_Zones_Repository.php';

class Flexible_Checkout_Fields_Conditional_Logic implements \FCFProVendor\WPDesk\PluginBuilder\Plugin\HookablePluginDependant {
	...
	public function hooks() {
		add_filter( 'flexible_checkout_fields_field_tabs', array( $this, 'flexible_checkout_fields_field_tabs' ),
		...
		), self::PRIORITY_BEFORE_DEFAULT, 4 );
		...
		add_filter( 'flexible_checkout_fields_condition', array(
			$this,
			'flexible_checkout_fields_condition_in_checkout_page'
		), 10, 2 );

		add_action( 'wp_ajax_flexible_checkout_fields_ajax', array( $this, 'wp_ajax_flexible_checkout_fields_ajax' ) );

		add_action( 'wp_ajax_retrieve_shipping_methods_by_selected_zone', array(
			$this,
			'retrieve_shipping_methods_by_selected_zone'
		) );
	}
	...
	public function admin_enqueue_scripts() {
		...
		if ( $current_screen->id == 'woocommerce_page_inspire_checkout_fields_settings' ) {
			...
			wp_enqueue_script( 'fcf_settings_saved_shipping_js',
				trailingslashit( $this->plugin->get_plugin_assets_url() ) . 'js/settings-saved-shipping-fields' . $suffix . '.js',
				array( 'jquery' ), $this->plugin->get_script_version()
			);

			wp_localize_script( 'fcf_settings_saved_shipping_js', 'fcf_settings_saved_shipping_js', [
	...
	public function flexible_checkout_fields_field_tabs( $tabs ) {
		...
		remove_filter( 'flexible_checkout_fields_field_tabs', array(
			$flexible_checkout_fields,
			'flexible_checkout_fields_field_tabs'
		) );
	...
	public function flexible_checkout_fields_field_tabs_content( $key, $name, $field, $settings ) {
		$flexible_checkout_fields = flexible_checkout_fields();
		remove_action( 'flexible_checkout_fields_field_tabs_content', array(
			$flexible_checkout_fields,
			'flexible_checkout_fields_field_tabs_content'
		), 10, 4 );
		...
		global $flexible_checkout_fields_all_field_values;
		global $flexible_checkout_fields_all_conditions;
		$shipping_zones           = Shipping_Zones_Repository::get_shipping_zones_by_name();
		$shipping_methods_no_zone = Shipping_Zones_Repository::get_active_shipping_methods_with_no_zone();
		$count                    = 0;
	...
	public function flexible_checkout_fields_field_tabs_content_js() {
		$flexible_checkout_fields = flexible_checkout_fields();
		remove_action( 'flexible_checkout_fields_field_tabs_content_js', array(
			$flexible_checkout_fields,
			'flexible_checkout_fields_field_tabs_content_js'
		), 10 );
		$count = 0;
	...
	/**
	 * Show shipping methods by selected zone
	 */
	public function retrieve_shipping_methods_by_selected_zone() {
		$response = array();
		if ( isset( $_POST['zone_id'] ) ) {
			if ( $_POST['zone_id'] === 'no_shipping_zones' ) {
				$shipping_methods = Shipping_Zones_Repository::get_active_shipping_methods_with_no_zone();
			} else {
				$shipping_methods = Shipping_Zones_Repository::get_list_of_shipping_methods_name_by_zone( (int) $_POST['zone_id'] );
			}

			$flexible_shipping_methods_id = array();
			foreach ( $shipping_methods as $method_id => $method_name ) {
				if ( strstr( $method_id, 'flexible_shipping' ) ) {
					$method_id                                  = str_replace( ':', '_', $method_id );
					$flexible_shipping_methods_id[ $method_id ] = $method_id;
				}
			}
			if ( ! empty( $shipping_methods ) ) {
				$response = array(
					...
					'flexible_shipping_ids'     => $flexible_shipping_methods_id,
				);
			}
