<?php
	...
	/**
	 * @var $flexible_checkout_fields_all_product_categories
	 * @var $flexible_checkout_fields_all_field_options
	 * @var $flexible_checkout_fields_all_field_values
	 * @var $flexible_checkout_fields_all_conditions
	 *
	 */
?>
...
jQuery(document).on('change', '.conditional-logic-shipping-fields', function() {
	if (jQuery(this).is(':checked')) {
		jQuery(this).parent().parent().parent().find('.fields-conditional-logic-shipping-fields').each(function() {
			jQuery(this).show();
		});
	}
	else {
...
jQuery(document).ready(function() {
            ...
            <div class="flexible_checkout_shipping_fields_add_rule" style="display:none;">
	            <fieldset>
		            <legend><?php printf( __( 'New Rule', 'flexible-checkout-fields-pro' ), '' ); ?></legend>
		                <div class="zones methods">
				            <div>
				                 <select required disabled class="zones" data-qa-id="field-conditional-logic-shipping-fields-rules-field"
				                 name="inspire_checkout_fields[settings][key][name][conditional_logic_shipping_fields_rules][-1][field]">
				                </select>
				                <select required disabled class="shipping_methods" data-qa-id="field-conditional-logic-shipping-fields-methods-field"
				                    name="inspire_checkout_fields[settings][key][name][conditional_logic_shipping_fields_rules][-1][value]">
				                   <option class="option_disabled" value="" selected
				                disabled><?php _e( 'Select Shipping Method', 'flexible-checkout-fields-pro' ) ?></option>
				                </select>
				            </div>
			            </div>
		            <div class="delete_rule">
