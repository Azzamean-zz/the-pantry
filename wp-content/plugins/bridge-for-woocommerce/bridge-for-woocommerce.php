...
function woo_bridge_fs_init()
{
    ...
    if ( woo_bridge_fs_is_parent_active_and_loaded() ) {
        ...
        if ( !woo_bridge_fs()->can_use_premium_code() ) {
            return;
        }
    } else {
...
if ( !class_exists( 'TC_WooCommerce_Bridge' ) ) {
    ...
    {
        var  $version = '1.2' ;
        var  $tc_version_required = '3.4.6.5' ;
        ...
        function __construct()
        {
            ...
            $alternative_hook_for_checkout_to_billing = apply_filters( 'tc_woo_bridge_alternative_hook_for_checkout_to_billing', false );
            if ( is_plugin_active( 'yith-woocommerce-multi-step-checkout/init.php' ) || is_plugin_active( 'woo-multistep-checkout/woo-multistep-checkout.php' ) ) {
                $alternative_hook_for_checkout_to_billing = apply_filters( 'tc_woo_bridge_alternative_hook_for_checkout_to_billing', true );
            }
            
            if ( $alternative_hook_for_checkout_to_billing ) {
                add_action( 'woocommerce_checkout_billing', array( &$this, 'add_standard_tc_fields_to_checkout' ), 99 );
            } else {
            ...
            add_action(
                ...
                array( &$this, 'tc_api_create_order' ),
                10,
                2
            );
            add_action(
            ...
            add_filter(
                'tc_paid_post_statuses',
                array( &$this, 'tc_modify_order_paid_statuses' ),
                10,
                1
            );
            add_filter(
            ...
            add_filter(
                'tc_available_checkins_per_ticket_field_name',
                array( &$this, 'tc_modify_available_checkins_per_ticket_field_name' ),
                10,
                1
            );
            add_filter(
            ...
            add_action(
                'woocommerce_pos_process_payment',
                array( &$this, 'tc_create_tickets_for_order' ),
                10,
                2
            );
            add_filter(
            ...
            add_filter(
                'tc_csv_admin_fields',
                array( &$this, 'modify_tc_csv_admin_fields' ),
                10,
                2
            );
            add_filter(
            ...
            add_filter(
                ...
                array( &$this, 'maybe_modify_product_is_purchasable' ),
                10,
                2
            );
            ...
            // Check upsell is activate or not
            ...
            if ( class_exists( 'Woocommerce_one_click_upsell_funnel' ) ) {
                ...
                add_action(
                    'woocommerce_pos_process_payment',
                    array( &$this, 'tc_create_tickets_for_order_without_upsell' ),
                    10,
                    2
                );
            }
            ...
            if ( !defined( 'TC_WOO_DISABLE_FRAGMENTS_UPDATE' ) ) {
                add_action(
                    ...
                    array( &$this, 'update_order_review_fragments' ),
                    10,
                    1
                );
            }
            ...
            add_action(
                ...
                array( &$this, 'add_order_line_item' ),
                10,
                3
            );
            add_action(
                'woocommerce_saved_order_items',
                array( &$this, 'update_order_line_items' ),
                10,
                2
            );
            add_action(
                'woocommerce_before_delete_order_item',
                array( &$this, 'delete_order_line_item' ),
                10,
                1
            );
        ...
        function tc_ticket_search_get_products( $query_vars )
        {
            ...
            if ( 'product' === $typenow && isset( $_GET['s'] ) && 'edit.php' === $pagenow ) {
                ...
                $search_results = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT {$wpdb->postmeta}.post_id as ticket_id FROM {$wpdb->posts} LEFT JOIN {$wpdb->postmeta} AS {$wpdb->postmeta} ON {$wpdb->posts}.ID = {$wpdb->postmeta}.meta_value WHERE {$wpdb->posts}.post_title LIKE '%" . implode( "','", $search_terms ) . "%' AND {$wpdb->postmeta}.meta_key = '" . $meta_key . "' ORDER BY {$wpdb->posts}.post_parent ASC, {$wpdb->posts}.post_title ASC" ) );
        ...
        <div class="tc_ticket_type_pd_variation_holder show_if_tc_ticket" <?php 
          ...
          <p class="form-field form-row form-row-first form-row-first tc_ticket_type_pd_variation">
            <label><?php 
            _e( 'Override "Check-ins per ticket" count', 'tc' );
            ?></label>
            <span class="woocommerce-help-tip" data-tip="<?php 
            echo  esc_attr( __( 'Set "Yes" if number of check-ins per ticket variation is different from the parent ticket product', 'tc' ) ) ;
            ?>"></span>
            <br /><input type="radio" name="_ticket_type_variation_override_checkins[<?php 
            echo  esc_attr( $loop ) ;
            ?>]" value="no" <?php 
          ...
          <p class="form-field form-row form-row-last tc_ticket_type_pd_variation_available_checkins_per_ticket_field">
            ...
            _e( 'Check-ins per ticket', 'tc' );
            ?></label>
            <span class="woocommerce-help-tip" data-tip="<?php 
            echo  esc_attr( __( 'Set number of check-ins allowed per ticket variation', 'tc' ) ) ;
            ?>"></span>
        ...
        /**
         * Save Ticket Instances when an item(s) is added in WC Order Line
         *
         * @param $item_id
         * @param $item
         * @param $order
         ...
         * @return null
         */
        function add_order_line_item( $item_id, $item, $order )
        {
            ...
            // WC Order line items
            $wc_cart_content = [];
            foreach ( $order->get_items() as $wc_item_id => $wc_item_object ) {
                $wc_cart_content[$wc_item_object->get_product_id()] = $wc_cart_content[$wc_item_object->get_product_id()] + $wc_item_object->get_quantity();
            }
            // Update Cart Contents
            update_post_meta( $order_id, 'tc_cart_contents', $wc_cart_content );
            // Ticket Instance Arguments
            ...
            for ( $x = 0 ;  $x < $item['quantity'] ;  $x++ ) {
                ...
                foreach ( $tc_tickets_instances as $tickets ) {
                    $tc_ticket_code = get_post_meta( $tickets->ID, 'ticket_code', true );
                    $tc_ticket_code = (int) explode( '-', $tc_ticket_code )[1];
                    if ( $tc_ticket_code ) {
                ...
                add_post_meta( $owner_record_id, 'ticket_type_id', $product_id );
                add_post_meta( $owner_record_id, 'ticket_code', $order_id . '-' . $available_slot );
                add_post_meta( $owner_record_id, 'event_id', $event_id );
                add_post_meta( $owner_record_id, 'item_id', $item_id );
            }
        }
         ...
         * @param $items
         */
        function update_order_line_items( $order_id, $items )
        {
            ...
            if ( 'woocommerce_save_order_items' == sanitize_text_field( $_POST['action'] ) && isset( $_POST['security'] ) ) {
                ...
                // WC Order line items
                $wc_cart_contents = [];
                $prev_tc_cart_contents = get_post_meta( $order_id, 'tc_cart_contents', true );
                foreach ( $order->get_items() as $wc_item_id => $wc_item_object ) {
                    $wc_product_id = $wc_item_object->get_product_id();
                    $is_ticket_instance = get_post_meta( $wc_product_id, '_tc_is_ticket', true );
                    // Collect Cart Contents
                    $wc_cart_contents[$wc_product_id] = (int) $items['order_item_qty'][$wc_item_id];
                    // Prepare item counts for comparison
                    $prev_count = $prev_tc_cart_contents[$wc_product_id];
                    $latest_count = (int) $items['order_item_qty'][$wc_item_id];
                    // Execute only if there is changes in quantities and item is a tickera ticket instance
                    
                    if ( $prev_count != $latest_count && $is_ticket_instance ) {
                        // Ticket Instance Arguments
                        $tc_tickets_instances_arg = array(
                            'post_parent'    => $order_id,
                            'post_type'      => 'tc_tickets_instances',
                        ...
                        // Get the absolute value of updated item quantity
                        $count_difference = abs( $prev_count - $latest_count );
                        
                        if ( $latest_count > $prev_count ) {
                            // Add Tickera Ticket Instance
                            ...
                            for ( $x = 0 ;  $x < $count_difference ;  $x++ ) {
                                ...
                                $available_slot = ( $available_slot ? reset( $available_slot ) : end( $tc_ticket_code_count ) + 1 );
                                $owner_record_id = @wp_insert_post( $arg, true );
                                add_post_meta( $owner_record_id, 'ticket_type_id', $wc_product_id );
                                add_post_meta( $owner_record_id, 'ticket_code', $order_id . '-' . $available_slot );
                                add_post_meta( $owner_record_id, 'event_id', $event_id );
                                add_post_meta( $owner_record_id, 'item_id', $wc_item_id );
                            }
                        } else {
                            // Remove Tickera Ticket Instance
                            $tc_tickets_instances = get_posts( $tc_tickets_instances_arg );
                            foreach ( $tc_tickets_instances as $instance_id ) {
                                
                                if ( $count_difference ) {
                                    // Delete Post
                                    wp_delete_post( $instance_id->ID );
                                    // Delete Postmeta
                ...
                // Update Cart Contents
                update_post_meta( $order_id, 'tc_cart_contents', $wc_cart_contents );
            }
         ...
         * @throws Exception
         */
        function delete_order_line_item( $item_id )
        {
            global  $wpdb ;
            $order_id = wc_get_order_id_by_order_item_id( $item_id );
            $order = wc_get_order( $order_id );
            ...
            $tc_tickets_instances = get_posts( $tc_tickets_instances_arg );
            foreach ( $tc_tickets_instances as $instance_id ) {
                $tc_product_id = get_post_meta( $instance_id->ID, 'ticket_type_id', true );
                $ticket_instance_item_id = get_post_meta( $instance_id->ID, 'item_id', true );
                $tc_cart_contents[$tc_product_id] = $tc_cart_contents[$tc_product_id] + 1;
                // IF: WC item_id exist in an instance
                
                if ( $ticket_instance_item_id == $item_id && $ticket_instance_item_id or $tc_product_id == $item->get_product_id() && !$ticket_instance_item_id ) {
                    // Delete Post
                    wp_delete_post( $instance_id->ID );
                    // Delete Postmeta
                    $wpdb->delete( $wpdb->prefix . 'postmeta', array(
            ...
            // Update Cart Contents
            update_post_meta( $order_id, 'tc_cart_contents', $tc_cart_contents );
        }
        ...
        {
            $tc_product_id = $object->get_id();
            
            if ( $object->get_type() == 'variation' ) {
                $tc_product_id = wp_get_post_parent_id( $object->get_id() );
            } else {
                $tc_product_id = $object->get_id();
            }
            
            if ( !TC_Ticket::is_sales_available( $tc_product_id ) ) {
                $status = false;
            }
            return $status;
        }
        ...
        public static function tc_create_tickets_for_order( $payment_details, $order )
        {
            ...
            foreach ( $order->get_items() as $item_id => $item ) {
                ...
                if ( $is_ticket ) {
                    $metas['ticket_type_id'] = $product_id;
                    //CHECK FOR VARIATIONS HERE!!!//////////////////////
                    $qty = $item_quantity;
                    for ( $i = 1 ;  $i <= $qty ;  $i++ ) {
                        ...
                        //count ticket after place order
                        $rowcount = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->posts} where post_parent = {$order_id} " ) );
                        
                        if ( $owner_record_num > $rowcount ) {
                            // check ticket after place order
                            $get_result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM `{$wpdb->postmeta}` WHERE `meta_value` LIKE '{$order_id}-1'" ), BOJECT );
                            $post_id = $get_result->post_id;
                            ...
                            update_post_meta( $owner_record_id, 'first_name', sanitize_text_field( $fname ) );
                            update_post_meta( $owner_record_id, 'last_name', sanitize_text_field( $lname ) );
        ...
        // end function
        //without upsell
        public static function tc_create_tickets_for_order_without_upsell( $payment_details, $order )
        {
            global  $woocommerce ;
            global  $wpdb ;
            $items = $order->get_items();
      ...
      <table class="<?php 
        ...
        <?php 
            ...
            foreach ( $ticket_types as $ticket_type ) {
                ...
                if ( $wc_catalog_visibility !== 'hidden' && $wc_catalog_visibility !== 'search' && TC_Ticket::is_sales_available( $ticket_type->ID ) == true ) {
                    $count_tickets++;
              ...
              <tr>
                ...
                <?php 
                        ...
                        ?>
                <td><?php 
                        echo  do_shortcode( '[add_to_cart id="' . $ticket_type->ID . '" style="" show_price="false"]' ) ;
                        ?></td>
              </tr>
              <?php 
                    } else {
    ...
    <?php 
        ...
        {
            $status = apply_filters( 'tc_seat_charts_get_reserved_seats_order_statuses_woo', array(
                'wc-completed',
                'wc-processing',
                'wc-pending',
                'wc-on-hold'
                ...
                if ( is_object( $order ) ) {
                    ...
                    if ( $order->get_status() == 'processing' || $order->get_status() == 'completed' ) {
                        ...
                        if ( $order_payment_method == 'cod' ) {
                            //if payment gateway is Cash on Delivery
                            
                            if ( $tc_woo_allow_cash_on_delivery_processing == 'yes' ) {
                                //If the processing payment status is allowed
                                $allowed_tickets_download = true;
                                //Allow tickets download and sending
                            } else {
                                
                                if ( $order->get_status() == 'completed' ) {
                                    //If the order status is completed allow download anyway
                                    $allowed_tickets_download = true;
                                } else {
                                    $allowed_tickets_download = false;
                                    //disallow download if it's processed and $tc_woo_allow_cash_on_delivery_processing is no
                                }
        ...
        function tc_redirect_ticket_single_to_event()
        {
            ...
            if ( is_single() ) {
                ...
                if ( $tc_woo_redirect_single_post_to_event == 'yes' ) {
                    ...
                    //make sure to redirect all products which are associated with a seating chart(s) to its event page
                    /* if (get_post_type($post->ID) == 'product') {
                               $product = wc_get_product($post->ID);
                               $$used_for_seatings_meta = get_post_meta($post->ID, '_tc_used_for_seatings', true);
                               $used_for_seatings = $$used_for_seatings_meta == 'yes' ? true : false;
                       
                               if ($used_for_seatings) {// && !$product->has_child()
                               $event_id = get_post_meta($post->ID, '_event_name', true);
                               wp_redirect(get_permalink($event_id));
                               exit;
                             }
        ...
        function tc_woocommerce_admin_order_data_after_order_details( $order )
        {
            ...
            if ( count( $tc_cart_info ) > 0 && !empty($tc_cart_info[0]['buyer_data']) ) {
                //Make sure that we have custom fields to show first
                $order_id = ( method_exists( $order, 'get_id' ) ? $order->get_id() : $order->id );
        ...
        function tc_validate_tickera_fields()
        {
            ...
            if ( isset( $this->tc_general_settings['email_verification_buyer_owner'] ) && 'yes' == $this->tc_general_settings['email_verification_buyer_owner'] ) {
                $owner_email = ( isset( $_POST['owner_data_owner_email_post_meta'] ) ? $_POST['owner_data_owner_email_post_meta'] : '' );
                $owner_confirm_email = ( isset( $_POST['owner_data_owner_confirm_email_post_meta'] ) ? $_POST['owner_data_owner_confirm_email_post_meta'] : '' );
                if ( $owner_email != $owner_confirm_email ) {
                    wc_add_notice( __( 'Attendees\' email addresses did not match.', 'woocommerce-tickera-bridge' ), 'error' );
                }
            }
        ...
        )
        {
            ...
            if ( !in_array( $email->id, $allowed_email_templates_ids ) ) {
                return;
            }
            ...
            if ( $order->has_status( 'processing' ) && $this->allowed_tickets_download( $order ) && !$sent_to_admin ) {
                ...
                if ( 'yes' == $send_processing_woo_order_message ) {
            ...
            if ( $order->has_status( 'completed' ) && $this->allowed_tickets_download( $order ) && !$sent_to_admin ) {
                ...
                if ( 'yes' == $send_completed_woo_order_message ) {
        ...
        function modify_settings_email_fields( $fields )
        {
            ...
            $fields = array_merge( $fields, array_values( $woo_processing_email_content ), array_values( $woo_completed_email_content ) );
            return $fields;
        }
  ...
  <?php 
        ...
        function tc_maybe_add_additional_tickets( $order_id, $order )
        {
            ...
            if ( isset( $_POST['add-ticket-submit'] ) && isset( $_POST['event_new'] ) && !empty($_POST['event_new']) && isset( $_POST['ticket_type_new'] ) && !empty($_POST['ticket_type_new']) ) {
                //make sure event and ticket type are selected
                //Add ticket instance
                $arg = array(
                ...
                if ( count( $tc_cart_info ) > 0 && !empty($tc_cart_info[0]['buyer_data']) ) {
                    ...
                    if ( class_exists( 'TC_Custom_Fields' ) ) {
                        //only add buyer data if custom forms is active
                        $forms = new TC_Forms();
  ...
  <?php 
    ...
    <?php 
            ...
            if ( defined( 'tc_woo_has_manual_tickets' ) && tc_woo_has_manual_tickets == true ) {
                ...
                if ( isset( $post_type ) && $post_type == 'shop_order' ) {
                    wp_enqueue_script(
                        ...
                        $this->plugin_url . 'js/admin-ticket.js',
        ...
        function tc_api_create_order( $order_id, $data )
        {
            ...
            $tc_tickets_instances = get_posts( $tc_tickets_instances_arg );
            foreach ( $tc_tickets_instances as $post ) {
                //make sure to delete tickets if exist already (failed and pending orders)
            ...
            foreach ( $items as $item => $values ) {
                ...
                if ( $is_ticket ) {
                    $metas['ticket_type_id'] = $values['product_id'];
                    //CHECK FOR VARIATIONS HERE!!!//////////////////////
                    $cart_contents[(int) $values['product_id']] = (int) $values['quantity'];
        ...
        /**
         * Create tickets upon creating a new order if needed
         *
         * @global mixed $woocommerce
         * @param int $order_id
         */
        function tc_order_created( $order_id )
        {
            ...
            if ( !empty($owner_data) ) {
                //execute bellow code only if it's related to tickera
        ...
        function tc_custom_settings_fields_save( $post_id )
        {
            ...
            if ( $_tc_is_ticket == 'yes' ) {
                ...
                if ( $_ticket_checkin_availability == 'time_after_first_checkin' ) {
        ...
        function tc_add_custom_settings()
        {
            ...
            if ( method_exists( 'TC_Ticket', 'is_sales_available' ) ) {
                ...
                woocommerce_wp_radio( array(
                    ...
                    'time_after_first_checkin' => 'Within the following time after first check-in',
                    'upon_event_starts'        => 'When the event starts',
                ),
                ...
                echo  '</div>' ;
                echo  '<div class="options_group" id="_ticket_checkin_availability_after_first_checkin">' ;
                $days_options = array();
        ...
        function add_standard_tc_fields_to_checkout()
        {
            ...
            if ( $cart_has_tickets ) {
                ...
                echo  '<div class="tickera_additional_info"></div>' ;
        ...
        function tc_csv_admin_fields_woo_billing( $fields )
        {
            ...
            $fields['col_woo_coupon_code'] = __( 'Coupon Code', 'tc' );
            //added woo coupan code csv export
            return $fields;
        }
        ...
        )
        {
            ...
            if ( isset( $post['col_woo_company_name'] ) ) {
                ...
                $new_export_array = array(
                    __( 'Company Name', 'tc' ) => $billing_company,
                );
            ...
            if ( isset( $post['col_woo_address_1'] ) ) {
                ...
                $new_export_array = array(
                    __( 'Address Line 1', 'tc' ) => $billing_address_line_1,
                );
                $export_array = array_merge( $export_array, $new_export_array );
            }
            ...
            if ( isset( $post['col_woo_address_2'] ) ) {
                ...
                $new_export_array = array(
                    __( 'Address Line 2', 'tc' ) => $billing_address_line_2,
                );
                $export_array = array_merge( $export_array, $new_export_array );
            }
            ...
            if ( isset( $post['col_woo_postcode'] ) ) {
                ...
                $new_export_array = array(
                    __( 'Postcode', 'tc' ) => $billing_postcode,
                );
                $export_array = array_merge( $export_array, $new_export_array );
            }
            ...
            if ( isset( $post['col_woo_order_total'] ) ) {
                $tc_check_post = get_post( $order->details->ID );
                
                if ( $tc_check_post->post_type !== 'tc_orders' ) {
                    $wc_order = new WC_Order( $order->details->ID );
                    $new_export_array = array(
                        __( 'Order Total', 'tc' ) => $wc_order->get_total(),
                    );
                    $export_array = array_merge( $export_array, $new_export_array );
                } else {
                    $cart_info = get_post_meta( $order->details->ID, 'tc_cart_info', true );
                    $new_export_array = array(
                        __( 'Order Total', 'tc' ) => $cart_info['total'],
                    );
                    $export_array = array_merge( $export_array, $new_export_array );
                }
            ...
            if ( isset( $post['col_woo_coupon_code'] ) ) {
                $order = new WC_Order( $order->details->ID );
                $coupons = $order->get_items( 'coupon' );
                foreach ( $coupons as $item_id => $item ) {
                    $new_export_array = array(
                        __( 'Coupan Code', 'tc' ) => $item->get_code(),
                    );
                    $export_array = array_merge( $export_array, $new_export_array );
                }
            }
    ...
    if ( is_multisite() && is_plugin_active_for_network( plugin_basename( __FILE__ ) ) ) {
        ...
        add_action( 'tets_fs_loaded', 'tc_woocommerce_bridge_load' );
    } else {
        $tc_woocommerce_bridge = new TC_WooCommerce_Bridge();
    }
...
add_filter(
    ...
    'tc_woobridge_modify_ticket_type_post_type_args',
    21,
    1
);
/**
