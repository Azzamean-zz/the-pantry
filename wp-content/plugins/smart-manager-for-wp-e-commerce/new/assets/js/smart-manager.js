...
Smart_Manager.prototype.init = function() {
	...
	this.loadMoreBtnHtml = "<button id='sm_editor_grid_load_items' style='height:2em;border: 1px solid #3892D3;background-color: white;border-radius: 3px;cursor: pointer;line-height: 17px;color:#3892D3;'> Load More <span>"+window.smart_manager.dashboard_display_title+"</span></button>"
...
Smart_Manager.prototype.load_dashboard = function() {
	...
	if( typeof(window.smart_manager.currentDashboardModel) == 'undefined' || window.smart_manager.currentDashboardModel == '' ) {
		...
		if( sm_dashboard_valid == 1 ) {			
			...
			var content = 'For managing '+ window.smart_manager.dashboard_display_title +', '+ window.smart_manager.sm_success_msg + ' <a href="' + window.smart_manager.pricingPageURL + '" target="_blank">Pro</a> version.';
...
Smart_Manager.prototype.event_handler = function() {
	...
	.off( 'click', '#sm_top_bar_action_btns_basic #sm_beta_move_to_trash, #sm_top_bar_action_btns_basic #sm_beta_delete_permanently').on( 'click', '#sm_top_bar_action_btns_basic #sm_beta_move_to_trash, #sm_top_bar_action_btns_basic #sm_beta_delete_permanently' ,function(){
		...
		if ( window.smart_manager.sm_beta_pro == 0 && window.smart_manager.selectedRows.length > window.smart_manager.sm_deleted_sucessfull ) {
			...
			let actionText = ( !window.smart_manager.trashEnabled || deletePermanently ) ? '<span style="color:#FF5B5E;">permanently delete</span>' : 'trash'; 
			...
			let all_text      = '<span style="font-size: 1.2em;">Are you sure you want to <strong>'+ actionText +' all</strong> the ' + window.smart_manager.dashboard_display_title + '?</span>';
