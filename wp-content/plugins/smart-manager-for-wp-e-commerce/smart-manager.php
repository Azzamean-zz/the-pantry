...
if ( defined('SMPRO') && SMPRO === true ) {
	...
	if ( ! class_exists( 'ActionScheduler' ) && file_exists( dirname( __FILE__ ). '/new/pro/libraries/action-scheduler/action-scheduler.php' ) ) {
		include_once 'new/pro/libraries/action-scheduler/action-scheduler.php';
	}
...
function sm_plugins_loaded() {
	...
	if ( defined('SMPRO') && SMPRO === true ) {
		...
		if ( !class_exists( 'StoreApps_Upgrade_3_6' ) ) {
			require_once 'pro/sa-includes/class-storeapps-upgrade-3-6.php';
		}
...
function sa_sm_dequeue_scripts() {
	global $wp_scripts;
...
add_action( 'admin_enqueue_scripts', 'sa_sm_dequeue_scripts', 999 );
...
function smart_manager_get_version() {
...
function smart_woo_add_modules_admin_pages() {
	...
	if( ( defined( 'SMBETAPRO' ) && SMBETAPRO === true ) || ( ( !empty( $current_user->roles[0] ) && $current_user->roles[0] == 'administrator' ) || ( !empty( $current_user_caps ) && $current_user_caps == 'administrator') ) ) {
		...
		if( !empty( $show_sa_plugins_page ) ) {
			add_submenu_page( 'smart-manager', __( 'StoreApps Plugins', 'smart-manager-for-wp-e-commerce' ),  __( 'StoreApps Plugins', 'smart-manager-for-wp-e-commerce' ), 'manage_options', 'sm-storeapps-plugins', 'sm_admin_page' );	
		}
