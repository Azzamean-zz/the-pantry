...
class Xoo_Wl_Core{
	...
	public function hooks(){
		...
		add_action( 'xoo_wl_send_back_in_stock_email_for_product', array( $this, 'cron_send_back_in_stock_email_for_product' ) );
		add_action( 'updated_postmeta', array( $this, 'send_email_on_stock_update' ) ,10 , 4 );
		add_action( 'xoo_wl_send_emails_on_joining_waitlist', array( $this, 'cron_send_emails_on_joining_waitlist' ) );
		add_action( 'xoo_wl_form_submit_success', array( $this, 'send_emails_on_joining_waitlist' ) );
	}
	...
	public function form_submit(){
		...
		try {
			...
			if( !xoo_wl_is_product_out_of_stock( $product_id ) ){
				$error->add( __( 'Product is already in stock, Please add to cart', 'waitlist-woocommerce' ) );
			}
			...
			$message = apply_filters( 'xoo_wl_form_submit_error_notice', $e->getMessage(), $e->getWpErrorCode() );
	...
	public function trigger_back_in_stock_email_for_product( $product_id ){
		...
		wp_schedule_single_event( time(), 'xoo_wl_send_back_in_stock_email_for_product', array(
			$product_id
		) );
	...
	public function send_emails_on_joining_waitlist( $inserted_id ){
		wp_schedule_single_event( time(), 'xoo_wl_send_emails_on_joining_waitlist', array(
			$inserted_id
		) );
	}


	public function cron_send_emails_on_joining_waitlist( $inserted_id ){

		//Admin notify
		if( xoo_wl_helper()->get_email_option('an-enable') === "yes" ){
			xoo_wl_emails()->adminNotify->trigger( $inserted_id );
		}

		//User notify
		if( xoo_wl_helper()->get_email_option('un-enable') === "yes" ){
			xoo_wl_emails()->userNotify->trigger( $inserted_id );
		}
	}
