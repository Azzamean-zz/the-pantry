jQuery(document).ready(function($){
	...
	$('.xoo-wl-exp-toogle').click(function(){
		var $cont = $(this).parents('.xoo-wl-exp-container');
		$cont.addClass('xoo-wl-exp-active');
	})

	//Export CSV form
	$('form.xoo-wl-exp-form').on( 'submit', function(e){

		e.preventDefault();

		var $btn 		= $(this).find('button[type="submit"]'),
			table_type 	= $(this).find('input[name="table_type"]').val(),
			formData 	= {
				action: 'xoo_wl_export_csv',
				table_type: table_type,
				checkboxes: {}
			},
			btnTxt 		= $btn.text(),
			$noticeCont = $('.xoo-wl-exp-notice');

		if( table_type === 'users_table' ){
			formData['product_id'] = $(this).find('input[name="product_id"]').val();
		}

		$btn.addClass('xoo-wl-loading').text( 'Exporting...Please wait.' );
		$noticeCont.html('');

		$(this).find('input[type="checkbox"]').each(function( index, el ){
			var $el = $(el);
			if( !$el.prop('checked') )  return;
			formData['checkboxes'][ $el.attr('name') ] = $el.val();
		});
		...
		$.ajax({
			...
			success: function(response){
				...
				$btn.removeClass('xoo-wl-loading');
			}
		});
	})
